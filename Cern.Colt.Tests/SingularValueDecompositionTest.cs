...
namespace Cern.Colt.Tests
{
    ...
    public class SingularValueDecompositionTest
    {
        ...
        public void TestLSI()
        {
            ...
            ClassicAssert.LessOrEqual(Math.Abs(scales[0] - 3.34), 0.01);
            ClassicAssert.LessOrEqual(Math.Abs(scales[1] - 2.54), 0.01);
            var terms = svd.U;
            ClassicAssert.LessOrEqual(Math.Abs(terms[0, 0] - 0.22), 0.01);
            ClassicAssert.LessOrEqual(Math.Abs(terms[1, 1] + 0.07), 0.01);
            var docs = svd.V;
            ClassicAssert.LessOrEqual(Math.Abs(docs[0, 0] - 0.20), 0.01);
            ClassicAssert.LessOrEqual(Math.Abs(docs[1, 1] - 0.17), 0.01);
        }
        ...
        public void TestUpdating()
        {
            ...
            ClassicAssert.AreEqual(j1.Assign(UnaryFunctions.Mult(k)), h);
            ...
            ClassicAssert.AreEqual(svd.S, s2);
        }
        ...
        public void IncrementalSVD()
        {
            ...
            foreach (var m in new[] { _a, _b })
            {
                ...
                for (int j = 0; j < s1.Length; j++) ClassicAssert.LessOrEqual(Math.Abs(s1[j] - s2[j]), Tolerance);
