...
public void init()
{
    ...
    int seed = 123456;
    current = 4 * seed + 1;
    density = A.cardinality() / (double)A.Count;
}
public void apply(cern.colt.Timer timer)
{
    int rows = B.rows();
...
protected static Boolean handle(String[] params)
{
	...
	if (cmd.Equals("dgemm")) bench_dgemm(params);
		...
		if (fun!=null) {
			...
			success = false;
			String s = "Command=" +params[0]+" is illegal or unknownd Should be one of "+commands()+"followed by appropriate parameters.\n"+usage()+"\nIgnoring this line.\n";
    ...
    { // batch mode, read commands from file
      ...
      */
        ...
        try
        {
            ...
            int token;
            while ((token = stream.nextToken()) != stream.TT_EOF)
            { // while not end of file
                if (token == stream.TT_EOL)
                { // execute a command line at a time
                  //System.out.println(words);
                    if (words.Count > 0)
                    { // ignore emty lines
                        String[] params = new String[words.Count];
                        for (int i = 0; i < words.Count; i++) params[i] = (String) words.Get(i);
...
protected static void run(double minSeconds, String title, Double2DProcedure function, String[] types, int[] sizes, double[] densities)
{
    ...
    for (int i = sizes.Length; --i >= 0;) rowNames[i] = int.ToString(sizes[i]);
	...
	*/
    System.out.println("Run took a total of " + runTime + "d End of run.");
}
/**
 * Executes procedure repeatadly until more than minSeconds have elapsed.
 */
