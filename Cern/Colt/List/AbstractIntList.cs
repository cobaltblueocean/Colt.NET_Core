// <copyright file="AbstractIntList.cs" company="CERN">
...
namespace Cern.Colt.List
{
    ...
    /// <summary>
    public abstract class AbstractIntList : AbstractList<int>
    {
        /// <summary>
        ...
        public virtual int[] Elements
        {
            ...
            {
                Clear();
                AddAllOfFromTo(new IntArrayList(value), 0, value.Length - 1);
            }
        }
        ...
        public override Boolean Contains(int elem)
        {
        ...
        public virtual AbstractIntList SetElements(int[] elements)
        {
            Clear();
        ...
        { //delta
            if (!(otherObj is AbstractIntList)) { return false; }
        ...
        /// <exception cref="IndexOutOfRangeException">index is out of range (<i>_size()&gt;0 && (from&lt;0 || from&gt;to || to&gt;=_size())</i>). </exception>
        public virtual void MergeSortFromTo(int from, int to, IntComparatorDelegate c)
        {
            int mySize = Size;
            CheckRangeFromTo(from, to, mySize);
        ...
        /// <see cref="Comparator"></see>
        /// <exception cref="IndexOutOfRangeException">index is out of range (<i>_size()&gt;0 && (from&lt;0 || from&gt;to || to&gt;=_size())</i>). </exception>
        public virtual void QuickSortFromTo(int from, int to, IntComparatorDelegate c)
        {
            int mySize = Size;
            CheckRangeFromTo(from, to, mySize);
        ...
        public virtual Boolean RemoveAll(AbstractIntList other)
        {
        ...
        public override void ReplaceFromWith(int from, ICollection<int> other)
        {
        ...
        /// <returns><code>true</code> if the receiver changed as a result of the call.</returns>
        public virtual Boolean RetainAll(AbstractIntList other)
        {
            if (other.Size == 0)
            {
        ...
        /// <exception cref="IndexOutOfRangeException">if <i>index &lt; 0 || index &gt;= _size()</i>. </exception>
        public virtual void Set(int index, int element)
        {
            if (index >= _size || index < 0)
                throw new IndexOutOfRangeException("Index: " + index + ", Size: " + _size);
