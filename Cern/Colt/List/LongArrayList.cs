...
namespace Cern.Colt.List
{
    ...
    public class LongArrayList : AbstractLongList
    {
        ...
        /// <summary>
        /// <param name="elements">the array to be backed by the the constructed list</param>
        public LongArrayList(List<long> elements)
        {
        ...
        public LongArrayList(int initialCapacity) : this(new long[initialCapacity])
        {
            SetSizeRaw(0);
        }
        /// <summary>
        ...
        public override void BeforeInsert(int index, long element)
        {
        ...
        public override int BinarySearchFromTo(long key, int from, int to)
        {
        ...
        {
            // overridden for performance only.
            LongArrayList clone = new LongArrayList((long[])_elements.Clone());
        ...
        public override long[] GetElements()
        {
            return _elements;
        }
        /// <summary>
        ...
        public override AbstractLongList SetElements(long[] elements)
        {
        ...
        { //delta
          // overridden for performance only.
          //if (!(otherObj is LongArrayList)) return base.Equals(otherObj);
            if (this == otherObj) return true;
            if (!(otherObj is LongArrayList))
            {
                if (otherObj is IList<long>)
                {
                    var other = (IList<long>)otherObj;
            ...
            else
            {
                ...
                var other = (LongArrayList)otherObj;
        ...
        public override Boolean ForEach(LongProcedureDelegate procedure)
        {
            // overridden for performance only.
        ...
        public override int IndexOfFromTo(long element, int from, int to)
        {
            // overridden for performance only.
            if (Size == 0) return -1;
        ...
        /// <exception cref="IndexOutOfRangeException">index is out of range (<i>Size&gt;0 && (from&lt;0 || from&gt;to || to&gt;=Size)</i>). </exception>
        public override AbstractLongList PartFromTo(int from, int to)
        {
            if (Size == 0) return new LongArrayList(0);

            CheckRangeFromTo(from, to, Size);

            var part = new long[to - from + 1];
        ...
        public override Boolean RemoveAll(AbstractLongList other)
        {
            // overridden for performance only.
            if (!(other is LongArrayList)) return base.RemoveAll(other);
        ...
        {
            // overridden for performance only.
            if (!(other is LongArrayList)) return base.RetainAll(other);
        ...
        /// <exception cref="IndexOutOfRangeException">index is out of range (index </exception>
        /// 		  &lt; 0 || index &gt;= Size).
        public override void Set(int index, long element)
        {
            // overridden for performance only.
            if (index >= Size || index < 0)
                throw new IndexOutOfRangeException("Index: " + index + ", Size: " + Size);
